...
export default function(e, wavesColor){
  ...
  if(wavesColor){
    ...
    if(typeof(wavesColor) === 'string'){
      types.forEach(type=>{
        if(wavesColor.indexOf(type) !== -1){
          ripple.style.background = wavesColor
        }
      })

      ripple.classList.add(`btn-${wavesColor}`)
    }
  }
